...
describe("validation surface — /health", () => {
    ...
    it("GET / returns the same shape as /health", async () => {
        const [a, b] = await Promise.all([worker.fetch(new Request("https://w/"), ENV), worker.fetch(new Request("https://w/health"), ENV)]);
...
describe("validation surface — hostname mismatch", () => {
    ...
    it("rejects success when EXPECTED_HOSTNAME does not match", async () => {
        vi.spyOn(globalThis, "fetch").mockImplementationOnce(async () => {
            return new Response(JSON.stringify({ success: true, hostname: "attacker.example.com", "error-codes": [] }), { status: 200 });
        });
